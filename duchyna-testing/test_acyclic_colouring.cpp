...
void test(std::string file, int max_k)
{
...
void find_edge_colouring_k_regular(std::string file, int time_limit)
{
    std::ifstream inputFile(file + ".g6");
    std::ofstream outputFile(file + "_edge.csv");
...
void find_vertex_colouring_k_regular(std::string file, int time_limit)
{
    std::ifstream inputFile(file + ".g6");
    std::ofstream outputFile(file + "_vertex.csv");
    ...
    while (std::getline(inputFile, line))
    {
        ...
        if (g[0].degree() == 3)
            k--;
        std::cout << k - 2 << " " << g.order() << " " << g.size() << " lg ";
        while (!has_acyclic_colouring(g, k))
        {
            std::cout << k << " ";
            k++;
...
void test_max_input(std::string file, int time_limit)
{
    std::ifstream inputFile(file + ".g6");
    std::ofstream outputFile(file + ".csv");

    outputFile << "graph,#vertices,#edges,max degree,acyclic edge chromatic index,duration" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    // find_acyclic_edge_chromatic_index(60);

    // find_acyclic_vertex_chromatic_index(60);
