 ...
 */
template <typename C>
concept Colouriser = requires(C c, const Graph &G)
{
    {c.isColourable(G)} -> std::convertible_to<bool>;
};

template <typename C, typename P>
concept EnumerativeColouriser = Colouriser<C> &&
        requires(C c, const Graph &G,
                 bool (*callback)(std::map<Edge, int>, P *param), P *callbackParam)
{
    {c.enumerateColourings(callback, callbackParam)} -> std::convertible_to<bool>;
};

template <typename C>
concept PrecolourColouriser = Colouriser<C> &&
        requires(C c, const Graph &G, const std::map<Edge, int> &precolouring)
{
    {c.isColourable(G, precolouring)} -> std::convertible_to<bool>;
};

template <typename C, typename P>
concept EnumerativePrecolourColouriser = PrecolourColouriser<C> &&
        requires(C c, const Graph &G,
                 bool (*callback)(std::map<Edge, int>, P *param), P *callbackParam,
                 const std::map<Edge, int> &precolouring)
{
    {c.enumerateColourings(callback, callbackParam)} -> std::convertible_to<bool>;
    {c.enumerateColourings(callback, callbackParam, precolouring)} -> std::convertible_to<bool>;
};
