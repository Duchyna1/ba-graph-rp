...
public:
	...
	inline Graph operator [](int i) { return read_graph6_line(graphStrings[i], initialF); }
	...
	inline Graph get(int i, Factory &f) {
		return read_graph6_line(graphStrings[i], f);
	}
...
/*
	...
	(if no factory is supplied, an individual one will be created for each graph).
