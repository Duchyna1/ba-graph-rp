...
concept CollectibleObject = requires(CO co, const CO &other, const std::string &s, bool do_inflate)
{
	{co.size()} -> int;
	{co.hash()} -> int;

	{co.compute_property()} -> void;
	{co.property_computed()} -> std::convertible_to<bool>;
	{co.property_value()} -> P;

	{co == other} -> std::convertible_to<bool>;

	{co.is_minimised()} -> std::convertible_to<bool>;
	{co.minimise()} -> void;
	{co.inflate()} -> void;
