...
// claw-free = does not contain K_{1,3} as an induced subgraph
inline bool is_claw_free(const Graph &G)
{
	for (auto &r : G) {
		auto ns = r.neighbours();
		for (auto n1: ns) {
			for (auto n2: ns) {
				if (n2 == n1) continue;
				for (auto n3: ns) {
					if (n3 == n1) continue;
					if (n3 == n2) continue;
					// there must be one of the edges between n1, n2, n3
					bool hasEdge = false;
					if (G.contains(Location(n1, n2))) hasEdge = true;
					if (G.contains(Location(n2, n3))) hasEdge = true;
					if (G.contains(Location(n3, n1))) hasEdge = true;
					if (!hasEdge) {
						// a claw was found
						return false;
					}
